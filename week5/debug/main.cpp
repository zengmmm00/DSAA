...
int main() {
    ...
    for(int i = 0; i < t; i++) {
        ...
        for(int j = 0; j < m; j++) {
            ...
            while(current -> ex < tmp -> ex && current1 -> ex < tmp -> ex && current1 != NULL) {//find the first position where ex > tmp -> ex
                current = current -> nxt;
                current1 = current1 -> nxt;
            }
            if(current -> ex == tmp -> ex) current -> co += tmp -> co;
            else {
                tmp -> nxt = current1;
            ...
            for(int j = 0; j < q; j++) {
                ...
                while(current -> ex < k && current != NULL){
                    current = current -> nxt;
                }
                if(current -> ex == k)
                    printf("%d ", current -> co);
